...
THEN
SIGNAL SQLSTATE '02000' SET MESSAGE_TEXT = 'Attempt to insert inconsistent data! User has to be null. Status:Deleted.';
END IF;
IF NEW.StatusId = deletedStatusId and NEW.PIN is not null
THEN
SIGNAL SQLSTATE '02000' SET MESSAGE_TEXT = 'Attempt to insert inconsistent data! PIN has to be null. Status:Deleted.';
END IF;
